...
husarion-snap-common@0.5.0 ──dump──►  $SNAP/usr/bin, $SNAP/usr/share/husarion-snap-common/config
...
3. **`husarion-snap-common` is pinned to tag `0.5.0`.** Provides validators, the `ros.env` generator, DDS XML, the `start/stop/restart_launcher.sh` wrappers. Editing its scripts requires changes in the external repo plus an optional pin bump — do not patch locally.
...
- `husarion-depthai` additionally has [check_daemon_running.sh](https://github.com/husarion/husarion-snap-common/blob/0.5.0/local-ros/check_daemon_running.sh) in its command-chain — it detects a live daemon and tells the user to stop it first.
...
- **`${SNAP_COMMON}` = `/var/snap/husarion-depthai/common/`** (cross-revision, NOT copied by snapd):
  - DDS XML (`dds-config-*.xml`) from snap-common — regenerated by `configure_hook_ros.sh` on every `ros.*` change.
...
1. Check upstream release notes:
   ...
   - [`luxonis/depthai-core`](https://github.com/luxonis/depthai-core/releases)
   — does the "1st startup after reboot" issue still exist (or have new regressions appeared)?
...
- After adding `dds-config-foo.xml` the user runs `sudo snap set husarion-depthai ros.transport=foo`. `validate_config_param` in `configure_hook_ros.sh` will catch a missing file.
...
- **Don't patch snap-common files locally** (`utils.sh`, `configure_hook_ros.sh`, `dds-config-*.xml`) — the next build overwrites everything. Edit the external repo and bump the pin.
...
- **Don't change the naming format of `camera-params-<NAME>.yaml` / `ffmpeg-params-<NAME>.yaml` / `dds-config-<NAME>.xml`** — all regexes in `validate_config_param` assume it.
