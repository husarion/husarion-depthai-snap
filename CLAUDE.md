...
- **Cascades `driver.depth=true`** — `configure` force-sets it via `snapctl set` whenever `pointcloud=true` and the model has stereo (rejects outright if it doesn't). Verified against upstream: `depthai-ros`'s own `pointcloud.launch.py`/`rgbd_pcl.launch.py` always pair `PointCloudXyzrgbNode` with an RGBD `params_file` (`pcl.yaml`/`rgbd.yaml`), and the node remaps its depth input from `<name>/stereo/image_raw` — a topic that doesn't exist without depth. So "pointcloud without depth" isn't a real, supported mode upstream either; the cascade just saves the operator from having to flip two switches for one outcome. The cascade is one-directional: setting `driver.depth=false` while `pointcloud` is still `true` gets silently re-forced back to `true` on the next `configure` run (logged, not an error) — turn `pointcloud` off first if you want depth off.
