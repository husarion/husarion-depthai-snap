...
- **`autogenerated`** (the default) — [generate_camera_params.sh](snap/local/generate_camera_params.sh), invoked by `configure` on **every** `snap set`, compiles `driver.depth`/`resolution`/`fps`/`imu`/`ir`/`ip`/`bitrate` into `${SNAP_DATA}/camera-params-autogenerated.yaml` from scratch, always the custom `husarion_depthai_pipeline` dual plugin (`RGBDual`/`RGBDDual` — never a stock `RGB`/`RGBD` type). No separate compose step to drift out of sync: the file IS the current `driver.*` state, always.
- **anything else** — a hand-authored file. `configure` detects `camera-params != autogenerated` and skips generation entirely (logs it, so it's never a silent no-op) — `depth`/`resolution`/`fps`/`imu`/`ir`/`ip`/`bitrate` stop having ANY effect, and that file is used completely verbatim. This is the escape hatch for anything the exposed keys can't express (asymmetric raw/encoder sizing, manual exposure for SLAM, hand-tuned `align_depth`/`lr_check`/`i_nn_type`, ...) — copy `camera-params-autogenerated.yaml` to a new name and hand-edit freely (README "Camera config").
...
- **One source of truth for cockpit too**: the `husarion-agent-extras` `camera` concern (`snap/husarion-agent-extras/config-seed/`) is a thin pass-through — its manifest fields map 1:1 onto these same `driver.*` keys via `snapctl set` (see `config-seed/hooks/camera`), and it force-pins `driver.camera-params=autogenerated` on every run so Manage always drives the generated file even if a headless `snap set` previously switched to a custom one. Its `RGB_RESOLUTION` manifest field default (`720P`) is kept in sync with `apply_defaults.sh` — if you change one, change the other. Same for the `RECTIFY_RGB` field (default `true`) added alongside it — Manage can now toggle the bottom of the rectify-rgb/depth/pointcloud chain directly (previously headless-only); the hook sets it before `depth`/`pointcloud` so a manifest apply converges on the depthai snap's own cascade rather than fighting it.

### `depth`/`resolution`/`fps`/`imu`/`ir`/`ip`/`bitrate` — what each key does
...
- `driver.bitrate` (default `3400000`) sets `rgb.i_low_bandwidth_bitrate` — the on-chip H.264 VideoEncoder's bitrate (bit/s), static CBR (see "On-chip H.264 bitrate is hardcoded, static CBR" below: not runtime-adjustable, a `snap set` restarts the daemon to apply it, same as any other `camera-params-autogenerated.yaml` key).
- **Capability-gated in `configure`**: `depth`/`pointcloud`/`imu`/`ir` are rejected outright against the wrong `driver.model` (same static per-model lists as `camera-probe.py`'s `MATRIX`) before `generate_camera_params.sh` ever runs. `resolution`/`fps`/`ip`/`bitrate` are NOT model-gated here (too many valid per-model combos to duplicate in bash) — an unsupported combo fails at the driver, not at `snap set` time; the cockpit's dynamic enum (fed by `camera-probe.py`'s per-model lists) keeps Manage from offering it.
...
### One camera per host — `driver.mxid` targeting was probe-only, and is now dead code (TODO: multi-camera?)

`snap/local/camera-probe.py`'s `probe_live()` still reads `HUSARION_AGENT_MXID` to pick which physical OAK to introspect when a host has more than one connected via USB (`dai.Device.getAllAvailableDevices()` can return several; without a selection it just takes `infos[0]`, i.e. whatever order `depthai` happens to enumerate). The cockpit `camera` concern's `MXID` manifest field that used to set that env var was removed in the 2026-08 camera-params-autogenerated redesign — so `HUSARION_AGENT_MXID` is now never set, `sel` in `probe_live()` is always empty, and this selection code is unreachable.

Two things worth knowing before touching this:

- **It was always probe-only.** Even before the field was removed, `driver.mxid` never flowed into the actual running pipeline — only into which device the *capability probe* queries for Manage's dynamic enums. Upstream `depthai_ros_driver` (`camera_param_handler.cpp`, v2.12.2-jazzy) genuinely supports targeting a specific physical device via `camera.i_mx_id` (serial) or `camera.i_usb_port_id` (USB port) — the same pattern as `camera.i_ip` for PoE — but this snap's `generate_camera_params.sh` has never written either. On a host with two OAKs plugged in, both the probe (pre-removal) and the driver (always) fell back to whatever `depthai` enumerates first — no guaranteed correspondence between them, and no way to pin the driver to one specific device across reboots/replugs.
- **This snap only ever drives one camera regardless.** One `driver.model`, one `generate_camera_params.sh` output, one `depthai.launch.py` invocation, one systemd daemon. "Multi-OAK" in the pre-2026-08 design (`SPEC-camera-config.md`, deleted this same redesign) meant "probe can disambiguate which of several connected devices you're configuring," never "run two OAK pipelines from one snap instance."

**Decision (2026-08): leave the dead code as-is for now** — no known robot or bench setup currently runs more than one OAK on a single host, so there's nothing to fix yet. **TODO before reviving or deleting it**: think through whether/how this snap should support multiple cameras — e.g. wiring a real `driver.mxid`/`driver.usb-port-id` key through to `camera.i_mx_id`/`i_usb_port_id` for deterministic single-device targeting, versus actually running N independent instances (parallel-installs? separate `ros.namespace` per instance?) for N physical cameras. Until that's decided, don't just restore the old cockpit `MXID` field — it would give the appearance of a fix without touching the actual driver-targeting gap above.
...
### On-chip H.264 bitrate (`driver.bitrate`) is static CBR — no runtime/adaptive reconfig

`driver.bitrate` (default `3400000`, added 2026-08) sets `i_low_bandwidth_bitrate` in `generate_camera_params.sh` — a `snap set` changes it and restarts the daemon, same as `resolution`/`fps`/etc. Two things worth knowing before assuming more is possible:

- **It's fixed CBR, not VBR.** `depthai::utils::VideoEncoderConfig` (the struct `rgb_dual.cpp`/`depth_dual.cpp` populate from `i_low_bandwidth_*` params) has no `rateCtrlMode` field at all — only `enabled`/`quality`/`profile`/`frameFreq`/`bitrate`. `dai::VideoEncoderProperties::rateCtrlMode` therefore stays at its library default, `CBR`. There is no VBR path through this wrapper.
- **`i_low_bandwidth_bitrate` is NOT runtime-settable, despite looking like a normal ROS param.** Verified against upstream `depthai-ros` source at the exact pinned tag (`v2.12.2-jazzy`, matching the installed `ros-jazzy-depthai-ros-driver` `2.12.2-1noble.20260616`): `SensorParamHandler::setRuntimeParams()` — the only mechanism that pushes ROS param changes into the live device via the `controlQ`/`CameraControl` path (see `RGBDual::updateParams`) — handles exclusively the `r_*` params (exposure/focus/whitebalance/sharpness/denoise/auto-exp-region). `i_low_bandwidth_bitrate` is read exactly once, in `rgb_dual.cpp`'s `setXinXout()`, at pipeline-build time. A `ros2 param set /oak i_low_bandwidth_bitrate <X>` (as opposed to `snap set husarion-depthai driver.bitrate=<X>`, which regenerates the file and restarts the daemon) changes only the ROS parameter's stored value — the already-built `dai::node::VideoEncoder` never sees it.
- **Consequence**: a genuine "adaptive bitrate depending on link quality" scheme (no daemon restart, reacting live) is not possible without new code — `driver.bitrate` only covers "pick a fixed number, apply it via a restart". Doing adaptive for real means adding a dedicated control input to the on-chip `VideoEncoder` in `husarion_depthai_pipeline` itself (a second `XLinkIn` feeding encoder reconfig, analogous to the existing `controlQ` for camera 3A) — non-trivial, not a param-wiring task. If this is ever requested, treat it as a real feature, not a quick key addition.
