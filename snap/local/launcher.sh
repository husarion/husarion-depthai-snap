...
# depth/resolution/fps/imu/ir/ip are NOT launch args anymore — they're baked
# into camera-params-autogenerated.yaml by generate_camera_params.sh (run from
# the configure hook), which the launch file loads verbatim via params_file.
# Only pointcloud stays a launch arg: it's a launch-topology decision (whether
# to load an extra composable node), not a depthai_ros_driver param.
OPTIONS=(
  name
  pointcloud
  rectify-rgb
...
# These bundled preset families were all deleted 2026-08 (see "camera-params:
# autogenerated vs custom" in CLAUDE.md) — this case can only still match a
# custom-named file a pre-2026-08 host happens to have kept under one of these
# exact names. Left in place for that legacy path; a fresh custom name is
...
for OPTION in "${OPTIONS[@]}"; do
  ...
  if [ "${NO_RAW_RGB}" = "true" ]; then
    case "${OPTION}" in
      rectify-rgb | pointcloud) VALUE="false" ;;
    esac
  fi
  if [ -n "${VALUE}" ]; then
