...
sudo snap set husarion-depthai driver.depth=true                # default: false — adds metric depth + on-chip H.264 disparity preview (rejected on OAK-1/OAK-1-LITE)
sudo snap set husarion-depthai driver.pointcloud=true           # default: false — PointCloudXyzrgbNode; implies (auto-sets) driver.depth=true
...
| `driver.depth` | `false` | switches the pipeline to also produce depth (metric raw + an on-chip H.264 disparity preview) — capability-gated on the camera having a stereo pair |
| `driver.pointcloud` | `false` | loads `PointCloudXyzrgbNode` — **implies `driver.depth=true`** (auto-set; verified against upstream `depthai-ros`, which never runs PCL without an RGBD pipeline — the node's depth input topic simply doesn't exist otherwise) |
